...
export default function UnderstandingOnePage() {
  ...
  const handleFinish = () => {
    if (answer.trim()) {
      ...
      localStorage.setItem('understanding_jawaban1', answer);
      router.push('/story/1');
    }
  };
  ...
  return (
    <div className="relative h-screen w-full overflow-hidden bg-slate-100">
      <Image
        src="/images/understanding/understanding1.png"
        alt="Understanding 1"
        fill
      ...
      <div className="absolute left-3 right-3 bottom-4 z-20 rounded-[32px] border-2 border-cyan-200 bg-white/95 p-4 shadow-[0_18px_50px_-30px_rgba(0,0,0,0.5)] ring-1 ring-slate-100">
        <div className="mb-3 rounded-[28px] bg-gradient-to-r from-sky-200 to-cyan-200 p-4 text-center shadow-inner shadow-cyan-200/20">
          <p className="text-sm font-extrabold text-slate-800">
            Isi pendapatmu tentang hak dan kewajiban sebelum melanjutkan cerita.
          </p>
        </div>
        ...
        <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:justify-between">
          <button
            type="button"
            onClick={() => router.push('/character/2')}
